...
async function getPromptButtons(browser) {
  return SpecialPowers.spawn(browser, [], async () => {
    ...
    const container = await ContentTaskUtils.waitForCondition(
      () => smartWindowElement.shadowRoot.querySelector("#browser-container"),
      "Wait for browser container"
    );

    const promptsElement = await ContentTaskUtils.waitForCondition(
      () => container.querySelector("smartwindow-prompts"),
